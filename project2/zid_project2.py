...
def read_prc_csv(tic):
    ...
    """
    path = os.path.join(cfg.DATADIR, f'{tic.lower()}_prc.csv')
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    df = pd.concat([read_prc_csv(os.path.join(cfg.DATADIR, tic))[prc_col] for tic in tickers], axis=1, join='outer')
    df.columns = tickers
    return df
...
def mk_ret_df(prc_df):
    ...
    """
    master = pd.DataFrame(index=pd.to_datetime([]))
    for tick, col in prc_df.items():
        master = pd.concat([master, col.pct_change()], axis=1, join='outer')

    mkt_prc = pd.read_csv(os.path.join(cfg.DATADIR, 'ff_daily.csv'), index_col='Date')['mkt']
    mkt_prc.index = pd.to_datetime(mkt_prc.index)
    master = pd.concat([master, mkt_prc], axis=1, join='inner')
...
def mk_aret_df(ret_df):
    ...
    """
    aret_df = ret_df.drop(columns='mkt') - ret_df['mkt'].values.reshape(-1, 1)
...
def get_ew_rets(df, tickers):
    ...
    """
    # Get df with selected tickers
    df = df[tickers]

    # Calculate mean across rows
    eqWeightedReturns = df.mean(axis=1, skipna=True)

    return eqWeightedReturns
...
def get_ann_ret(ser, start, end):
    ...
    """
    # Filter to dates required
    filteredSet = ser[start:end]

    # Get total gross return
    tot_ret = (1 + filteredSet).prod()

    # Days with NON-MISSING RETURNS
    N = filteredSet.count()
...
# lowercase_tickers = [tic.lower() for tic in cfg.TICKERS]
#
# # Creates a data frame containing returns
# returnsDF = mk_prc_df(lowercase_tickers, prc_col='adj_close')
# returnsDF = mk_ret_df(returnsDF)
# returnsDF.index = pd.to_datetime(returnsDF.index)

# # Create a dictionary (ticker:avg return) to store the tickers and their daily values
# ticRetDic ={}
#
# # Get averages for each ticker
# for ticker in lowercase_tickers:
#     # Returns avg value of a col for a given year
#     avgReturn = get_avg(returnsDF, ticker, 2020)
#     ticRetDic[ticker] = avgReturn
#
# max_ticker = max(ticRetDic, key=ticRetDic.get)
# print(max_ticker)
...
#
# # Get ew df
# eqDf = get_ew_rets(returnsDF, lowercase_tickers)
#
# annRet = get_ann_ret(eqDf, '2010-01-01', '2020-12-31')
#
# print(annRet)
...
# tsla_series = returnsDF['tsla']
# print(tsla_series)
#
# annRet = get_ann_ret(tsla_series, '2010-01-01', '2020-12-31')
#
# print(annRet)
...
returnsDF = mk_prc_df(lowercase_tickers, prc_col='adj_close')
returnsDF = mk_ret_df(returnsDF)
returnsDF = mk_aret_df(returnsDF)
returnsDF.index = pd.to_datetime(returnsDF.index)
print(returnsDF)
