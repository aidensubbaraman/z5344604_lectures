...
def get_tics(pth):
    ...
    with open(pth, 'r') as tickersFile:
        # loop through each line in file
        for tickerText in tickersFile:
            # Remove white space and split into respective data fields
            components = tickerText.strip().split("=")
            # When correctly split
            if len(components) == 2:
                exchange = components[0].strip().lower().replace('"', '')
                tic = components[1].strip().lower().replace('"', '')
                # Check conditions - variables exist, and are lowercase
                if exchange and tic and exchange.isalpha() and tic.isalpha():
                    tickerDictionary[tic] = exchange
...
def line_to_dict(line):
    ...
    for col in COLUMNS:
        # Initialise width for the column
        width = COLWIDTHS[col]
        # Find value and remove white space
        value = line[start: start+width].strip()
        # Set dic value
        newDict[col] = value
        # Reset start location for next col
        start += width
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    """
    # When tickers_lst is not empty
    if tickers_lst is not None:
        # Check for is list is empty
        if not tickers_lst:
            raise Exception('tickers_lst is an empty list')
        # Check for if ticker is not in ticker dic
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
        ...
        # valid col name check
        for colName in col_lst:
            if colName not in COLUMNS:
                raise Exception(f"{colName} is not a valid column")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    # When tickers_lst == None, dictionary returned should contain the data for all tickers found in tic_exchange_dic
    ...
    for ticker in tickers_lst:
        # Read data for ticker
        linesList = read_dat(ticker)

        # Initialise dictionary for ticker data
        ticker_data = {
            'exchange': tic_exchange_dic[ticker],
            'data': []
        }

        # Loop through lines for the ticker's data
        for line in linesList:
            # Converting line to dictionary
            line_dict = line_to_dict(line)

            if col_lst is not None:
                # Filter the line_dict to only include columns from col_lst
                line_dict = {col: line_dict[col] for col in col_lst}

            # Populate the data fields with the new dicts
            ticker_data['data'].append(line_dict)

        # Add to final dict
        finalDic[ticker] = ticker_data
